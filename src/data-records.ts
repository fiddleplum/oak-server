...
// 			return cache.dataFile!.dataRecords;
// 		}
...
// 			}
// 			const resultOfAnds = cache.dataFile!.dataRecords.splice(0);
...
// 		}
// 		// Get the data record.
// 		const result = await this._getDataRecordCacheAndIndex(table, id, false);
// 		if (result.cache !== undefined && result.found) {
// 			return result.cache.dataFile!.dataRecords[result.index];
// 		}
// 		else {
...
// 					if (result.found) {
// 						result.cache.dataFile!.dataRecords[result.index] = dataRecord as DataRecord;
// 					}
// 					else {
// 						result.cache.dataFile!.dataRecords.splice(result.index, 0, dataRecord as DataRecord);
// 					}
...
// 			if (result.cache !== undefined && result.found) {
// 				result.cache.dataFile!.dataRecords.splice(result.index, 1);
// 				result.cache.dirty = true;
// 			}
// 		}
// 	}
...
// 		}
// 		const cache = await this._loadCache(table, id, createCacheIfNotFound);
// 		if (cache !== undefined) {
// 			const index = this.getIndexOfDataRecord(id, cache.dataFile!.dataRecords, indexOfId);
// 			return {
// 				cache: cache,
// 				index: index,
// 				found: (index < cache.dataFile!.dataRecords.length && cache.dataFile!.dataRecords[index][indexOfId] === id)
// 			};
// 		}
// 		else {
// 			return {
...
// 				newCache.dataFile = JSON.parse(data.toString('utf-8'));
...
// 	private async _saveCache(filename: string, cache: Cache): Promise<void> {
// 		console.log(`Saving cache "${filename}".`);
// 		try {
// 			await fs.promises.writeFile(filename, JSON.stringify(cache.dataFile));
// 			cache.dirty = false;
...
// 	// /** Resolves with true if the user has the proper permissions. */
// 	// private async hasPermissions(user: string, permissions: Permissions, type: 'edit' | 'view'): Promise<boolean> {
// 	// 	// Get the data record.
// 	// 	const result = await this._getDataRecordCacheAndIndex('groups', user, false);
// 	// 	if (result.cache !== undefined && result.found) {
// 	// 		const groupDataRecord = result.cache.dataFile!.dataRecords[result.index];
// 	// 		const groups = (groupDataRecord[1] as string).split(',');
// 	// 	}
// 	// 	return false;
// 	// }
...
// /** The permissions property at the root of every data file. */
...
// /** The contents of every data file. */
// interface DataFile {
// 	/** Permissions. */
