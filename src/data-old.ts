...
export class Data {
	constructor(config: Config, folder: string) {
		...
		for (const tableName in this._config.tables) {
			...
			// If it doesn't have a binning function, default to single file 'data.json'.
			else {
	 ...
	 * Returns a promise that resolves with an array of data records when they are all loaded. */
	async list(table: string): Promise<DataRecord[]> {
